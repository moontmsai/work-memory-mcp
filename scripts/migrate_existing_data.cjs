...
try {
  ...
  projectGroups.forEach(group => {
    console.log(`  📁 ${group.project_name}: ${group.memory_count}개 (${group.earliest_date} ~ ${group.latest_date})`);
  });
  ...
  const migration = db.transaction(() => {
    ...
    for (const group of projectGroups) {
      ...
      // 세션 ID 생성
      const sessionId = `session_${group.project_name.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
      ...
      `).run(
        ...
        `Auto-generated session for migrating ${group.memory_count} existing memories`,
        'completed', // 기존 데이터이므로 완료된 세션으로 처리
        true, // 자동 생성
        group.memory_count,
        group.memory_count, // 메모리 수만큼 활동으로 간주
        'migration_script'
      );
  ...
  const sessionStats = db.prepare(`
    SELECT 
      ...
      s.project_name,
      s.status,
      COUNT(m.id) as linked_memories
    FROM work_sessions s
    LEFT JOIN work_memories m ON s.session_id = m.session_id
    GROUP BY s.session_id
    ORDER BY linked_memories DESC
  `).all();
  
  sessionStats.forEach(stat => {
    console.log(`  🔗 ${stat.session_id}`);
    console.log(`     프로젝트: ${stat.project_name}`);
    console.log(`     연결된 메모리: ${stat.linked_memories}개`);
  ...
  sampleJoin.forEach((sample, index) => {
    ...
    console.log(`     세션 ID: ${sample.session_id}`);
  });
